...
import { blogJsonLd, createMetadata, seoPages } from "../../lib/seo";

export const metadata = createMetadata(seoPages.blog);
      ...
      <main className="font-blog-sans mx-auto flex min-h-screen w-full max-w-[1060px] flex-col px-5 py-12 sm:px-8 lg:py-16">
        ...
        <header className="mb-12 max-w-[760px]">
          ...
          <p className="font-blog-serif mt-5 text-[20px] leading-8 text-base-content/70">
            Technical writing, project notes, and engineering lessons from the
            systems I build across backend, frontend, and AI products.
          </p>
        </header>
        ...
        {posts.length === 0 ? (
          <section className="rounded-2xl border border-base-300 bg-base-200/45 p-8">
            ...
            <p className="mt-3 max-w-[620px] text-base-content/65">
              The blog is ready to read from the backend API. Published articles
              will appear here once the API returns data from the CMS.
            </p>
          </section>
        ) : (
          <section className="grid gap-7">
            {posts.map((post) => (
              ...
              >
                <Link href={`/blog/${post.slug}`} className="grid gap-0 md:grid-cols-[280px_1fr]">
